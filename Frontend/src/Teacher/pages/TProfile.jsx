...
function TeacherProfile() {
  ...
  useEffect(() => {
    if (teacherUser) {
      setFormData({
        ...
        ssn: teacherUser.ssn || "",
        joiningYear: teacherUser.joiningYear || "",
      });
    }
  }, [teacherUser]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Save teacher profile:", formData);
          ...
          <div className="w-full md:w-72 bg-slate-900 p-6 flex flex-col items-center">
            <div className="relative">
              <div className="w-28 h-28 rounded-full overflow-hidden border-2 border-blue-500">
                <img
                  src={teacherUser.profile_pic || "https://via.placeholder.com/150"}
                  alt="Profile"
              ...
              >
                ...
                <input
                  ...
                  className="hidden"
                  accept="image/*"
                  // TODO: handle avatar file change
                />
              </label>
            </div>
            ...
            <p className="text-slate-400 text-sm">{teacherUser.email}</p>
            <div className="mt-6 w-full space-y-3">
              <div className="flex items-center gap-2 text-slate-400">
                <Mail className="w-4 h-4 text-blue-500" />
                <span className="text-sm">
                  {teacherUser.email ? "Email Verified" : "Not Verified"}
                </span>
              </div>
              <div className="flex items-center gap-2 text-slate-400">
                <BookOpen className="w-4 h-4 text-blue-500" />
                <span className="text-sm">{teacherUser.ssn}</span>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, joiningYear: e.target.value })
                  }
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-blue-500 transition-colors"
                  placeholder="2023"
                  min="2000"
