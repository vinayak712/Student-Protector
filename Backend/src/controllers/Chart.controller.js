...
export const getGradesData = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching grades data" });
  }
};
...
export const getFiles = async (req, res) => {
  try {
    const studentId = req.user._id;
    const files = await File.find({ student: studentId }).sort({
      createdAt: -1,
    });
...
export const getMessages = async (req, res) => {
  try {
    ...
    const messages = await Message.find({
      $or: [{ student: studentId }, { recipient: studentId }],
    })
      .sort({ timestamp: 1 })
      .populate({
        path: "student",
...
export const getStudentByUSN=async function getStudentByUSN(req, res) {
  try {
